...
public class GetAprilTag {
    ...
    public boolean Distance(LinearOpMode opMode) {
        boolean targetFound = false;    // Set to true when an AprilTag target is detected

        initAprilTag(opMode);

        if (USE_WEBCAM)
            setManualExposure(6, 250, opMode);  // Use low exposure time to reduce motion blur

        // Wait for the driver to press Start
        opMode.telemetry.addData("Camera preview on/off", "3 dots, Camera Stream");
    ...
    */
    private void setManualExposure(int exposureMS, int gain, LinearOpMode opMode) {
