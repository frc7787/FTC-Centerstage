...
@Config
public class CalibratePropDetector implements VisionProcessor, CameraStreamSource {
    ...
    private PropLocation determinePosition(Point center) {
        PropLocation tempPosition = NONE;

        if (center.x < 100) {
            tempPosition = NONE;
        } else if (center.x > 180) {
            tempPosition = RIGHT;
        } else {tempPosition = LEFT;}

        return tempPosition;
    } // end of determinePosition

    // Returns an enum being the current position where the robot will park
    public PropLocation getPosition() {
        return position;
    } // end of getPosition
